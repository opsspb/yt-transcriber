...
- `pkg-config` available in `PATH` (required to build the PyAV dependency on macOS/Linux). Install it with `brew install pkg-config` on macOS or `sudo apt-get install pkg-config` on Debian/Ubuntu. If you already have working wheels and want to skip the preflight check, set `YT_DIARIZER_ALLOW_MISSING_PKG_CONFIG=1`.
